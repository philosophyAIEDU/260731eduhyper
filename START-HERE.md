...
```
  ...
  ②  낭독 녹화           OBS 로 PART 1 을 읽어서 녹화
        ↓
  ③  한 폴더에 모으기     녹화본 + 기획서 3장  (한글 폴더여도 OK)
        ↓
...
HyperFrames 는 한글 경로에서 **에러 메시지도 없이 그냥 죽습니다.**
모르면 원인을 못 찾고 몇 시간을 헤맵니다. 실제로 그랬습니다.

다만 **당신이 신경 쓸 일은 없습니다.** 붙여넣을 프롬프트에
"영어 경로에 작업 폴더를 만들어서 거기서 작업해줘" 라고 이미 적혀 있습니다.

```
녹화본·기획서가 있는 폴더   →  한글이어도 됩니다 (바탕화면 OK)
Claude 가 작업하는 폴더     →  영어. Claude 가 알아서 만듭니다
```
