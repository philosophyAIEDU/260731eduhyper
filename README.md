...
모르면 원인을 못 찾고 몇 시간을 헤맵니다. (실제로 87분을 날린 적이 있습니다)

```
❌ C:\Users\내이름\바탕화면\양자컴퓨터 영상\     → 조용히 실패
✅ C:\Users\내이름\hyperframes-work\quantum\    → 정상
```

**녹화본과 기획서는 한글 폴더에 있어도 괜찮습니다.** ffmpeg 과 Whisper 는 한글 경로를
정상 처리합니다. HyperFrames 가 작업하는 폴더만 영어면 됩니다.

붙여넣을 프롬프트에 이 내용이 이미 들어 있어서, 실제로는 신경 쓸 일이 없습니다.
...
■ 작업 폴더
내 자료 폴더는 한글이라 HyperFrames 가 거기서는 못 돌아.
(한글 경로에서 에러 메시지도 없이 죽어)
C:\Users\사용자명\hyperframes-work\ 아래에 영어 이름으로 폴더를 만들고,
이 스타터 키트를 복사해서 거기서 작업해줘.
완성된 영상만 마지막에 내 원래 폴더로 옮겨주면 돼.
